...
public class UserController {
    ...
    public Utilisateur getOneLogin(String email) {
        System.out.println("------>public Utilisateur getOneLogin(String email )"+email);
        Query q = em.createQuery("select u from Utilisateur u where u.login =:login");
        q.setParameter("login", email.toLowerCase().trim());
    ...
    public Utilisateur getUtilisateurById(int idUtilisateur) {
        Query q = em.createQuery("SELECT r from Utilisateur r WHERE r.id =:idUtilisateur");

        q.setParameter("idUtilisateur", idUtilisateur);
        List<Utilisateur> ql = q.getResultList();
    ...
    public Utilisateur majUtilisateur(Utilisateur u, String email, String nom, String prenom, String numTel, String Role, List<Etablissement> etablissements) {
        if (email != null && util.validator.validateEmail(email)) {
            u.setLogin(email.toLowerCase());
            
        }
        System.out.println("email "+util.validator.validateEmail(email));
    ...
    public Utilisateur majUtilisateur(Utilisateur u, String email, String nom, String prenom, String numTel, String Role, String[] idEtabs) {
        return majUtilisateur(u, email, nom, prenom, numTel, Role,ec.getEtablissementsById(idEtabs) );
    }

    public Utilisateur getUtilisateurById(String strid) {
        int idUtilisateur = -1;
        try {
            idUtilisateur = Integer.parseInt(strid);
        } catch (Exception e) {
            System.err.println(strid + "is not an id (getUtilisateurByStrId)");
            return null;
        }
        return getUtilisateurById(idUtilisateur);
    }
